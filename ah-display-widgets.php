<?php
/*
Plugin Name: AH Display Widgets
Plugin URI: https://andreas-hecht.com/ah-display-widgets/
...
class AH_Display_Widgets extends WP_Widget {
    
    
    var $transient_name = 'ah_dw_details';
    var $checked = array();
    var $id_base = '';
    var $number = '';
    ...
    public function __construct() {
        ...
        $hook = apply_filters( 'ah_dw_callback_trigger', 'wp_loaded' );
        ...
        register_activation_hook( dirname(__FILE__) . '/ah-display-widgets.php', array( &$this, 'delete_transient' ) );

        add_action( 'plugins_loaded', array( &$this, 'load_lang' ) );

        // get custom Page Walker
        $this->page_list = new AH_DW_Walker_Page_List();
    }

    
    public function trigger_widget_checks() {
        add_filter( 'sidebars_widgets', array( &$this, 'sidebars_widgets' ) );
    }

    
    
    public function show_widget( $instance ) {
        
        $instance['ah_dw_logged'] = self::show_logged( $instance );

        // check logged in first
        if ( in_array( $instance['ah_dw_logged'], array( 'in', 'out' ) ) ) {
            
            $user_ID = is_user_logged_in();
            if ( ( 'out' == $instance['ah_dw_logged'] && $user_ID ) || ( 'in' == $instance['ah_dw_logged'] && ! $user_ID ) ) {
                return false;
            }
        }
        ...
        $show = apply_filters( 'ah_dw_instance_visibility', $show, $instance );
        ...
        $instance['ah_dw_include'] = isset( $instance['ah_dw_include'] ) ? $instance['ah_dw_include'] : 0;
        
        

        if ( ( $instance['ah_dw_include'] && false == $show ) || ( 0 == $instance['ah_dw_include'] && $show ) ) {
            
            return false;
            
        } else if ( defined('ICL_LANGUAGE_CODE') && $instance['ah_dw_include'] && $show && ! isset( $instance[ 'lang-' . ICL_LANGUAGE_CODE ] ) ) {
    ...
    public function sidebars_widgets( $sidebars ) {
        
        if ( is_admin() ) {
            return $sidebars;
        }

        global $ah_wp_registered_widgets;
        ...
        foreach ( $sidebars as $s => $sidebar ) {
            ...
            foreach ( $sidebar as $w => $widget ) {
                ...
                if ( ! isset( $ah_wp_registered_widgets[ $widget ] ) ) {
                    continue;
                }
                ...
                if ( isset( $this->checked[ $widget ] ) ) {
                    ...
                    $opts = $ah_wp_registered_widgets[ $widget ];
    ...
    public function hidden_widget_options( $widget, $return, $instance ) {
...
?>
    <div class="dw_opts">
        <input type="hidden" name="<?php echo esc_attr( $widget->get_field_name('ah_dw_include') ); ?>" id="<?php echo esc_attr( $widget->get_field_id('ah_dw_include') ); ?>" value="<?php echo esc_attr( $instance['ah_dw_include'] ) ?>" />
        <input type="hidden" id="<?php echo esc_attr( $widget->get_field_id('ah_dw_logged') ); ?>" name="<?php echo esc_attr( $widget->get_field_name('ah_dw_logged') ); ?>" value="<?php echo esc_attr( $instance['ah_dw_logged'] ) ?>" />
    ...
    public function show_widget_options() {
    ...
    public function show_hide_widget_options( $widget, $return, $instance ) {
        self::register_globals();
...
?>
        <p>
            <label for="<?php echo esc_attr( $widget->get_field_id('ah_dw_include') ); ?>">
                <?php _e( 'Show Widget for:', 'display-widgets' ) ?>
            </label>
            <select name="<?php echo esc_attr( $widget->get_field_name('ah_dw_logged') ); ?>" id="<?php echo esc_attr( $widget->get_field_id('ah_dw_logged') ); ?>" class="widefat">
                <option value="">
                    <?php _e( 'Everyone', 'display-widgets' ) ?>
                </option>
                <option value="out" <?php echo selected( $instance[ 'ah_dw_logged'], 'out' ) ?>>
                    <?php _e( 'Logged-out users', 'display-widgets' ) ?>
                </option>
                <option value="in" <?php echo selected( $instance[ 'ah_dw_logged'], 'in' ) ?>>
                    <?php _e( 'Logged-in users', 'display-widgets' ) ?>
                </option>
            </select>
        </p>

        <p>
            <select name="<?php echo esc_attr( $widget->get_field_name('ah_dw_include') ); ?>" id="<?php echo esc_attr( $widget->get_field_id('ah_dw_include') ); ?>" class="widefat">
                <option value="0">
                    <?php _e( 'Hide on checked pages', 'display-widgets' ) ?>
                </option>
                <option value="1" <?php echo selected( $instance[ 'ah_dw_include'], 1 ) ?>>
                    <?php _e( 'Show on checked pages', 'display-widgets' ) ?>
                </option>
            </select>
        </p>

        <div style="height:150px; overflow:auto; border:1px solid #dfdfdf; padding:5px; margin-bottom:5px;">
            <h4 class="ah_dw_toggle" style="cursor:pointer;margin-top:0;"><?php _e( 'Miscellaneous', 'display-widgets' ) ?> +/-</h4>
        ...
        ?>
                    <p>
                        <input class="checkbox" type="checkbox" <?php checked( $instance[ 'page-' . $key ], true ); ?> id="
                        <?php echo esc_attr( $widget->get_field_id('page-'. $key) ); ?>" name="
                            <?php echo esc_attr( $widget->get_field_name('page-'. $key) ); ?>" />
                                <label for="<?php echo esc_attr( $widget->get_field_id('page-'. $key) ); ?>">
                                    <?php echo $label; ?>
                                </label>
                    </p>
                    <?php
            ...
            <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Pages') ?> +/-</h4>
            <div class="dw_collapse">
                <?php 
        ...
        ?>
            ...
            <?php if ( ! empty( $this->cposts ) ) { ?>
                <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Custom Post Types', 'display-widgets' ) ?> +/-</h4>
                <div class="dw_collapse">
                    <?php
        ...
        ?>
                        <p>
                            <input class="checkbox" type="checkbox" <?php checked( $instance[ 'type-'. $post_key], true ); ?> id="
                            <?php echo esc_attr( $widget->get_field_id('type-'. $post_key) ); ?>" name="
                                <?php echo esc_attr( $widget->get_field_name('type-'. $post_key) ); ?>" />
                                    <label for="<?php echo esc_attr( $widget->get_field_id('type-'. $post_key) ); ?>">
            ...
            unset( $post_key, $custom_post );
                ...
                <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Custom Post Type Archives', 'display-widgets' ) ?> +/-</h4>
                <div class="dw_collapse">
                    <?php
                foreach ( $this->cposts as $post_key => $custom_post ) {
        ...
        ?>
                        <p>
                            <input class="checkbox" type="checkbox" <?php checked( $instance[ 'type-' . $post_key . '-archive' ], true ); ?> id="
                            <?php echo esc_attr( $widget->get_field_id( 'type-'. $post_key . '-archive' ) ); ?>" name="
                                <?php echo esc_attr( $widget->get_field_name( 'type-' . $post_key . '-archive' ) ); ?>" />
                                    <label for="<?php echo esc_attr( $widget->get_field_id( 'type-' . $post_key . '-archive' ) ); ?>">
                                        <?php echo stripslashes( $custom_post->labels->name ) ?>
                                            <?php _e( 'Archive', 'display-widgets' ) ?>
                                    </label>
                        </p>
                        <?php } ?>
                </div>
                <?php } ?>
                    ...
                    <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Categories') ?> +/-</h4>
                    <div class="dw_collapse">
                        <?php $instance['cat-all'] = isset( $instance['cat-all'] ) ? $instance['cat-all'] : false; ?>
                            <p>
                                <input class="checkbox" type="checkbox" <?php checked( $instance[ 'cat-all'], true ); ?> id="
                                <?php echo esc_attr( $widget->get_field_id('cat-all') ); ?>" name="
                                    <?php echo esc_attr( $widget->get_field_name('cat-all') ); ?>" />
                                        <label for="<?php echo $widget->get_field_id('cat-all'); ?>">
                                            <?php _e( 'All Categories', 'display-widgets' ); ?>
                                        </label>
                            </p>
                            <?php
        ...
        ?>
                                <p>
                                    <input class="checkbox" type="checkbox" <?php checked( $instance[ 'cat-'. $cat->cat_ID], true ); ?> id="
                                    <?php echo esc_attr( $widget->get_field_id('cat-'. $cat->cat_ID) ); ?>" name="
                                        <?php echo esc_attr( $widget->get_field_name('cat-'. $cat->cat_ID) ); ?>" />
                                            <label for="<?php echo $widget->get_field_id('cat-'. $cat->cat_ID); ?>">
        ...
        ?>
                    ...
                    <?php if ( ! empty( $this->taxes ) ) { ?>
                        <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Taxonomies', 'display-widgets' ) ?> +/-</h4>
                        <div class="dw_collapse">
                            <?php
        ...
        ?>
                                <p>
                                    <input class="checkbox" type="checkbox" <?php checked( $instance[ 'tax-'. $tax], true ); ?> id="
                                    <?php echo esc_attr( $widget->get_field_id('tax-'. $tax) ); ?>" name="
                                        <?php echo esc_attr( $widget->get_field_name('tax-'. $tax) ); ?>" />
                                            <label for="<?php echo esc_attr( $widget->get_field_id('tax-'. $tax) ); ?>">
                                                <?php echo str_replace( array( '_','-' ), ' ', ucfirst( $taxname ) ) ?>
                                            </label>
                                </p>
                                <?php
            unset( $tax );
        ...
        ?>
                        ...
                        <?php } ?>
                            ...
                            <?php if ( ! empty( $this->langs ) ) { ?>
                                <h4 class="ah_dw_toggle" style="cursor:pointer;"><?php _e( 'Languages', 'display-widgets' ) ?> +/-</h4>
                                <div class="dw_collapse">
                                    <?php
        ...
        ?>
                                        <p>
                                            <input class="checkbox" type="checkbox" <?php checked( $instance[ 'lang-' . $key ], true ); ?> id="
                                            <?php echo esc_attr( $widget->get_field_id('lang-'. $key) ); ?>" name="
                                                <?php echo esc_attr( $widget->get_field_name('lang-'. $key) ); ?>" />
                                                    <label for="<?php echo esc_attr( $widget->get_field_id('lang-'. $key) ); ?>">
        ...
        ?>
                                ...
                                <?php } ?>
                                    ...
                                    <p>
                                        <label for="<?php echo esc_attr( $widget->get_field_id('other_ids') ); ?>">
                                            <?php _e( 'Comma Separated list of IDs of posts not listed above', 'display-widgets' ) ?>:</label>
    ...
    public function update_widget_options( $instance, $new_instance, $old_instance ) {
        self::register_globals();
        ...
        $instance['ah_dw_include'] = ( isset( $new_instance['ah_dw_include'] ) && $new_instance['ah_dw_include'] ) ? 1 : 0;
        $instance['ah_dw_logged'] = ( isset( $new_instance['ah_dw_logged'] ) && $new_instance['ah_dw_logged'] ) ? $new_instance['ah_dw_logged'] : '';
...
?>
            <script type="text/javascript">
                ...
                jQuery(document).ready(function($) {
                    $('.widgets-holder-wrap').on('click', '.ah_dw_toggle', ah_dw_toggle);
                });
                ...
                function ah_dw_toggle() {
                    jQuery(this).next('.dw_collapse').toggle();
                }
                /*]]>*/
    ...
    public function show_logged( $instance ) {
        if ( isset( $instance['ah_dw_logged'] ) ) {
            return $instance['ah_dw_logged'];
        }

        if ( isset( $instance['dw_logout'] ) && $instance['dw_logout'] ) {
            $instance['ah_dw_logged'] = 'out';
        } else if ( isset( $instance['dw_login'] ) && $instance['dw_login'] ) {
            $instance['ah_dw_logged'] = 'in';
        } else {
            $instance['ah_dw_logged'] = '';
        }
    ...
    public function page_types(){
        ...
        return apply_filters('ah_dw_pages_types_register', $page_types);
    }
...
*/
class AH_DW_Walker_Page_List extends Walker_Page {
...
new AH_Display_Widgets();

/*
custom Page Walker CSS
*/
function ah_dw_widgets_style() {
    echo '<style>';
...
add_action( 'admin_print_styles-widgets.php', 'ah_dw_widgets_style' );
